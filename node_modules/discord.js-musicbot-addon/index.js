 ...
 *							loop: Loop command. (default: loop)
 */
module.exports = function (client, options) {
	...
	const LOOP = (options && options.loop) || 'loop';
	var musicbot = (client, options);
	...
	if (typeof LOOP !== 'string') {
		console.log(new TypeError('loop must be a string'));
		process.exit(1);
	}
	 ...
	 */
	 function musichelp(msg, suffix) {
		 if (!suffix) {
			...
			.setColor(0x27e33d)
			msg.channel.send({embed});
		} else if (suffix.includes(LOOP)) {
			const embed = new Discord.RichEmbed()
			.setAuthor(`${PREFIX}${LOOP}`, client.user.avatarURL)
